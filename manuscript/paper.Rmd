...
permutations from $\hat{r}$ that are "closer" to $r$.
...
defined, $m$, is used to indicate which ranks are salient. We call these
the "salient weights" to differentiate from the other weighting
function. As an example, for a four-team tournament, if the organizer
decides that only the winner is important, then $m = \{1,0,0,0\}$ (i.e.
essentially measuring effectivity). While here we only consider
...
![This chart compares the normalized mutual information (on log10 scale)
of various tournament structures against the number of top teams being
considered, where the team strengths are equally distanced based on a
normal distribution.
\label{fig:fig1}](images/Normal_Information_Curve_Graph.png){width="408"}

Based on the Figure 1 \ref{fig:fig1} , the intuitive best tournament,
...
![Plot to further explore the difference between single elimination and
double elimination competition structures
\label{fig:fig2}](images/tournament_comparison_plot_se_de.png){width="409"}

Double elimination is slightly better than single elimination, with the
gap growing wider as the top number of teams looked at increases.
However, those are under the assumption that the true strengths were
ordered correctly in the seeding. While double elimination was expected
to be slightly better than single elimination, the increase in the gap
is likely due to noise that comes with how ties are handled. Ties led to
