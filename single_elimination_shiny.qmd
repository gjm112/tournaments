...
simulate_single_elimination_tournament <- function(num_teams, distribution, seeding_structure = NULL, ties = T, series = 1, third_place = T, reseeded = F) {
    ...
    while (length(teams) > 1) {
      ...
      if (reseeded && length(teams) > 1) {
        ...
        if (n_valid > 1) {
          ...
          # Add back any NA teams (byes)
          teams <- c(reseeded_teams, na_teams)
        }
      }
...
test <- simulate_single_elimination_tournament(num_teams = 8, distribution = "Same", ties=F, series=1, third_place=T, seeding_structure = c(1,2,3,4,5,6,7,8), reseeded = TRUE)
test
