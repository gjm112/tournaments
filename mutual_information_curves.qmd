...
weighted_kendall <- function(rhat, k, wtype = "symmetric_salience", salient_weights = NULL, r = NULL) {
  n <- length(rhat)
  if (is.null(r)) r <- 1:n
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i != j && w[i, j] != 0) {
        # Skip uninformative pairs (ties in either vector)
        if (r[i] == r[j] || rhat[i] == rhat[j]) next
  
        denom <- denom + abs(w[i, j])
        summ <- summ + w[i, j] * sign(r[i] - r[j]) * sign(rhat[i] - rhat[j])
      }
    }
  }
  
  # Handle case where denom = 0 (e.g., all ties)
  if (denom == 0) return(1)
...
weighted_kendall(c(1,1,1,1,5,6), k = 6, r = c(1,1,1,1,5,6))
