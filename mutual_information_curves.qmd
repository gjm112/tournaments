...
perms_list <- permn(1:5)

# Apply weighted_kendall to each permutation
results <- sapply(perms_list, function(p) {
  weighted_kendall(rhat = p, k = 5, wtype = "symmetric_salience")
})

# Put into a dataframe
kendall_df <- data.frame(
  permutation = sapply(perms_list, paste, collapse = ""),
  weighted_kendall = results
)

kendall_df

sum(kendall_df$weighted_kendall)
mean(kendall_df$weighted_kendall)
```
