...
weighted_kendall(rhat=c(2,1,3,4), k=4)
...
mut_info <- function(sims, num_teams, rank_weights = NULL, wtype = "symmetric_salience"){
  ...
  # Extract rank_hat lists, widen into columns y1..yN
  y_df <- sims %>%
    group_by(simulation) %>%
    summarise(rank_hat = list(rank_hat), .groups = "drop") %>%
    mutate(rank_hat = map(rank_hat, unlist)) %>%
    unnest_wider(rank_hat, names_sep = "_")

  colnames(y_df)[-1] <- paste0("y", 1:num_teams)

  # Probability of each observed permutation
  prob_df <- y_df %>%
  ...
  Raj_dist <- numerator/denominator
...
indices = 1:8,
"Single Elimination (Reseeded)" = mi_se_reseeded,
"Group Stage" = mi_group_stage,
"Double Elimination" = mi_de_norm,
"Single Elimination (Set Structure)" = mi_se_norm,
"Coin Flip Single Elimination" = mi_random,
#"Unif_SE" = mi_se_unif,
"Bad Seeding Structure Single Elimination" = mi_bad,
"Coin Flip Round Robin" = mi_perms,
"Round Robin" = mi_test_rr,
"Worst Possible" = mi_worst,
"Best Possible" = mi_best,
check.names = FALSE
)
...
## MI Calculation and Plotting

These functions can take a list of dataframes and create mutual information that can then be plotted.
...
mi_ex <- compute_mi_for_datasets(list(
   singleElimination_normal_noTies_1 = singleElimination_normal_noTies_1,
   singleElimination_normal_noTies_1_badseed = singleElimination_normal_noTies_1_badseed
 ))
...
plot_mi <- function(mi_results, dataset_names = NULL) {
  ...
  # Create plot
  ggplot(plot_df, aes(x = index, y = mi_values, color = dataset, group = dataset)) +
    geom_line(linewidth = 1) +
    geom_point(size = 2) +
    labs(
      title = "Mutual Information Comparison",
      x = "Top K Teams",
      y = "Mutual Information (MI)",
      color = "Dataset"
    ) +
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
    scale_x_continuous(breaks = 1:max(plot_df$index)) +
    theme_minimal() +
    theme(
...
Round Robin - with m rounds (1,2,4)

Stepladder- 8 vs 7, winner plays 6, etc.

Waterfall - stepladder round robin

Group Stage - winners of group advance, top 2 advance

Staged Round Robin - top 2 in each group form new group, bottom 2 new group (each new group is a new round robin)
