...
\urlstyle{same}
\hypersetup{
  pdftitle={Assessing Ranking Fidelity of Competition Structures via Weighted Mutual Information},
  pdfauthor={Zachary Culp; Josie Peterburs; Ryan P. A. McShane; Gregory J. Matthews},
  pdfkeywords={tournament structures, mutual information},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\date{March 12, 2026}
\title{\bf Assessing Ranking Fidelity of Competition Structures via
Weighted Mutual Information}
\author{
Zachary Culp\thanks{The authors gratefully acknowledge \emph{please
...
\noindent%
{\it Keywords:} tournament structures, mutual information
\vfill
...
\citet{Goosens2024}, \citet{Appleton1995-zn} and \citet{Csato2021}.
...
the efficacy of the structure (\citet{Lasek2018-de},
\citet{Sziklai2022-kf}). If one were only interested in correctly
ranking the true best competitor as first, with the ranks of the
remaining competitors viewed as inconsequential, we refer to this here
are effectivity (\citet{Glenn1960-mn}). While there are many competition
structures where efficacy and effectivity are useful
measures\footnote{efficacy is a useful measure for the EPL, for example,
  as the ranking of the teams matters at both the high end (i.e.~entry
  into special tournaments) and the low end (i.e.~relegation status) and
  effectivity is a useful measure for the NCAA College Basketball
  tournament (i.e.~main goal is to crown a champion)}, there are many
other competitions where the correct ranking of more than one
...
discussed in \citet{Johnson2022-wz}.
...
Specifically, we start with the concept of mutual information
(\citet{Guiasu1977-nu}).
...
\subsection{Weighted Mutual
Information}\label{weighted-mutual-information}

In order to alleviate this problem, we instead consider \emph{weighted}
mutual information (\citet{Guiasu1977-nu}). We want to give more weight
to permutations from \(\hat{r}\) that are ``closer'' to \(r\).
Essentially, this problem comes down to choosing a metric that measures
the closeness between the vectors \(\hat{r}\) and \(r\). One then gives
more weight to combinations of \(\hat{r}\) and \(r\) that are closer and
less weight to combinations that are far apart. A natural choice for
distance between vectors the sum of squared deviations between the two
vectors. Specifically, we define the loss function between \(\hat{r}\)
and \(r\) to be \(l(r, \hat{r}) = \sum_{i=1}^n (\hat{r}_i - r_i)^2\).
Then, since small losses should be associated with large weights, we
focus on the recipricol of this loss function.
...
\subsection{Salient Weights (and
ranks)}\label{salient-weights-and-ranks}
...
\citet{Shieh1998} with a slight modification to the sign function.
...
rule detailed in \citet{Cover2006}, \[
...
{\centering \includegraphics[width=7.08333in,height=\textheight,keepaspectratio]{images/combined_plot.png}
...
{\centering \includegraphics[width=7.08333in,height=\textheight,keepaspectratio]{images/combined_combinations_plot.png}
...
{\centering \includegraphics[width=7.08333in,height=\textheight,keepaspectratio]{images/single_elimination_plot.png}
...
{\centering \includegraphics[width=7.08333in,height=\textheight,keepaspectratio]{images/round_robin_plot.png}
